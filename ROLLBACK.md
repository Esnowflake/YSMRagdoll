...
发行分支：`release/0.5.26`；源码标签：`0.5.26`。本版基于 `5ceaea7` 的最终牵引修订升级版本号，
安装包为 `build/libs/ysmragdoll-0.5.26-all.jar`。完整发行说明见 `releases/0.5.26.md`。
需要回退到之前的公开版本时，从 GitHub 的 0.5.25 Release 下载对应 `-all.jar`，退出游戏后替换；
不要同时安装两个版本。源码可用 `git switch --detach 0.5.25` 查看，先保存未提交修改。
下面的本地修订记录和备份路径作为历史保留。

## 发布前本地修订：0.5.25 抓取点牵引与柔软关节
